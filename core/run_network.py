...
def run_simulation( neural_network, tfinal, **args ):
    """
...
def construct_network():
    ...
    neuralNetwork.construct_coupling_matrices()
    # initial conditions (linspace, not random)
    neuralNetwork.initvec = rand_init_vector( neuralNetwork.EDIM, neuralNetwork.IDIM )
    return neuralNetwork


def run( theNetwork ):
    """
    options object is global
    """
    # set up and run the simulation
    #cell_input = make_input_dict( theNetwork )
    soln = run_simulation( theNetwork,
                           options.tfinal )

    # if necessary, move tmp time series to proper file
    # should always have a /var/tmp folder, right??
    # TEMP = '/data/jberwald/neurons/tmp_data/neurons_tmp'
    TEMP = '/var/tmp/neurons_tmp'
    ...
    numpy.savetxt( options.savedir + 'C_ei', theNetwork.C_ei )
    numpy.savetxt( options.savedir + 'C_ie', theNetwork.C_ie )
    numpy.savetxt( options.savedir + 'C_ii', theNetwork.C_ii )

    # masked arrays for finding the mean
    M_ei = numpy.ma.masked_equal( theNetwork.C_ei, 0 )
    M_ie = numpy.ma.masked_equal( theNetwork.C_ie, 0 )
    M_ii = numpy.ma.masked_equal( theNetwork.C_ii, 0 )
...
if __name__ == "__main__":
    ...
    # get on with running a simulation
        ...
        neuralNetwork = construct_network()
