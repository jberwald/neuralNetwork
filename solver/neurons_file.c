...
void initneurons_file(void)
{
  (void) Py_InitModule("neurons_file", neuronsMethods);
  import_array();
}
 ...
 *network_fitness():
   ...
   Output    -- time vector returned by ode solver.
...
static PyObject *network_fitness(PyObject *self, PyObject *args)
{
  ...
  FILE *fh;
  const char *tmpfile = "/data/jberwald/neurons/tmp_data/neurons_tmp"; //"/data/tmp/neuronsTmp";
 ...
 tmpvec will hold ODE solution at each step. œ*/
  ...
  /* average initial excitatory state */
  if (TREE==1) { 
    /* If tree structure, only average the excitatory cells in the
       top cluster */
    EX_LOOP = 5;
  }
  else {
  ...
  fprintf( fh, "%g ", t );
  fprintf( fh, "%g ", temp );
  /* **** Record inhib voltage here if needed  */
  ...
  int solver_state = 0;
   while (t < tfinal)
	...
	break;
      ...
      /* excitatory voltage averages */
      temp = 0.0;
      for (i=0; i < EX_LOOP; i++) {
	temp = (1/(double) EX_LOOP) * Cstate[i];
      }
      
      /* Write voltage avg to file */
      fprintf( fh, "%f\n", temp );
